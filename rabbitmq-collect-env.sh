    ...
    then
        ...
        then
            printf "[DEBUG] preserving output directory '%s'\\n" \
                   "$output_dir"
        elif [ -n "$RABBITMQ_COLLECT_ENV_TEMP_DIR" ]
        then
            # User specified temp dir - inform them about cleanup
            printf "[INFO] temporary artifacts preserved in: '%s'\\n" \
                   "$output_dir"
        else
            # Default behavior: clean up temp directory
...
Examples:
  # Use default temporary locations
  rabbitmq-collect-env

  # Specify output directory for final tarball
  RABBITMQ_COLLECT_ENV_OUTPUT_DIR=/var/tmp rabbitmq-collect-env

  # Specify both output and temp directories
  RABBITMQ_COLLECT_ENV_OUTPUT_DIR=/var/tmp \
  RABBITMQ_COLLECT_ENV_TEMP_DIR=/scratch \
  rabbitmq-collect-env

  # Keep temp artifacts for debugging
  RABBITMQ_COLLECT_ENV_TEMP_DIR=/tmp/debug rabbitmq-collect-env -vvv
EOF
}
...
{
    # Determine working directory for intermediate artifacts
    if [ -n "$RABBITMQ_COLLECT_ENV_TEMP_DIR" ]
    then
        # User specified a temp directory via environment variable
        if [ ! -d "$RABBITMQ_COLLECT_ENV_TEMP_DIR" ]
        then
            # Directory doesn't exist, try to create it
            mkdir_or_die "$RABBITMQ_COLLECT_ENV_TEMP_DIR"
        fi

        # Check if directory is writable
        if [ ! -w "$RABBITMQ_COLLECT_ENV_TEMP_DIR" ]
        then
            printf "[ERROR] temp directory '%s' is not writable\\n" \
                   "$RABBITMQ_COLLECT_ENV_TEMP_DIR" 1>&2
            exit 1
        fi

        # Create a timestamped subdirectory to avoid conflicts
        output_dir="$RABBITMQ_COLLECT_ENV_TEMP_DIR/rabbitmq-collect-$$-$(date '+%Y%m%d-%H%M%S')"
        mkdir_or_die "$output_dir"

        printf "[INFO] temporary artifacts will be stored in: '%s'\\n" \
               "$RABBITMQ_COLLECT_ENV_TEMP_DIR"

        if [ $verbose -gt 2 ]
        then
            printf "[DEBUG] using user-specified temp directory: '%s'\\n" \
                   "$RABBITMQ_COLLECT_ENV_TEMP_DIR"
            printf "[DEBUG] created working directory: '%s'\\n" \
                   "$output_dir"
        fi
    else
        # Default behavior: use mktemp
        output_dir="$(mktemp -d)"
        if [ $verbose -gt 2 ]
        then
            printf "[DEBUG] created temporary working directory: '%s'\\n" \
                   "$output_dir"
        fi
    fi
...
collect_rabbitmq_info()
{
    ...
    readonly with_timeout="timeout 120"
    ...
    pid=$($with_timeout rabbitmqctl eval 'os:getpid().' | sed 's/"//g')
...
collect_overview_info()
{
    ...
    rabbitmq_version="$(rabbitmqctl eval '{_,_,V}=proplists:lookup(rabbit,application:which_applications()),V.' | sed 's/"//g')"
    erlang_version="$(rabbitmqctl eval 'erlang:system_info(otp_release).' | sed 's/"//g')"
    {
...
build_output_archive()
{
    ...
    if [ -n "$RABBITMQ_COLLECT_ENV_OUTPUT_DIR" ]
    then
        # User specified an output directory via environment variable
        printf "[INFO] output archive will be created in: '%s'\\n" \
               "$RABBITMQ_COLLECT_ENV_OUTPUT_DIR"

        if [ ! -d "$RABBITMQ_COLLECT_ENV_OUTPUT_DIR" ]
        then
            # Directory doesn't exist, try to create it
            mkdir_or_die "$RABBITMQ_COLLECT_ENV_OUTPUT_DIR"
        fi

        # Check if directory is writable
        if [ ! -w "$RABBITMQ_COLLECT_ENV_OUTPUT_DIR" ]
        then
            printf "[ERROR] output directory '%s' is not writable\\n" \
                   "$RABBITMQ_COLLECT_ENV_OUTPUT_DIR" 1>&2
            exit 1
        fi

        output_archive_dir="$RABBITMQ_COLLECT_ENV_OUTPUT_DIR"
        if [ $verbose -gt 2 ]
        then
            printf "[DEBUG] using user-specified output directory: '%s'\\n" \
                   "$output_archive_dir"
        fi
    else
        # Default behavior: use temporary directory
    ...
    if [ $verbose -gt 2 ]
    then
        echo "[DEBUG] running command 'tar -C \"$output_dir\" -zcf \"<output_archive>\"'"
    else
        printf '.'
    fi
